...
sb11_22 <- read.csv("Examen_Saber_11_20202_entrenamiento.txt", sep = ";")
...
LAFT$codmpio <- c_gen %>% filter(ano==2018) %>% select(codmpio)
...
proprul<-c_gen %>% filter(ano==2021) %>% group_by(coddepto) %>% summarise(porc_rul = 100*sum(pobl_rur)/sum(pobl_tot)) 
